<template>
  <v-container>
    <v-card class="mt-5" max-height="auto">
      ...
      <v-card-text  v-if="this.getFavResponse != 'none' && this.getStateFavs != null">
        <v-list three-line>
          <template v-for="(item, index) in this.getStateFavs">
...
export default {
  ...
  methods:{
    ...
    ...mapActions({
      getFavs: "getFavs",
      getFavPage: 'getFavPage'
    }),
  },
  computed:{
    ...mapGetters({
      getStateFavs: 'getStateFavs',
      getFavResponse: 'getFavResponse',
      getFavPagination: 'getFavPagination'
    })
  },
  watch:{
    getStateFavs: function() {
      //listeing to changes in a vuex state
      if (this.getStateFavs != "" && this.getStateFavs != null) {
        this.pagination = this.getFavPagination
        this.diaload = false
      }
    },
     getFavResponse: function() {
      //listeing to changes in a vuex state
      if (this.getFavResponse == "none") {
        this.diaload = false
      }
    },
  },
